...
ModelHike is not a code generator that uses AI. It is an **intent compiler** and a **deterministic compilation target for AI**. The `.modelhike` file is the source of truth between human intent and production code. Any AI can write to it. ModelHike guarantees the output.
